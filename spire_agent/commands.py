"""Shared business logic for Slay the Spire 2 SpireBridge commands.

This module provides async command functions that accept a ``SpireBridgeClient``
and return human-readable strings.  Both the MCP server and the CLI delegate to
these functions so that behaviour is identical across surfaces.
"""
...
def _fmt_card(card: dict[str, Any]) -> str:
    """Format a card dict as a compact human-readable string."""
    parts = [card.get("name", "?")]
    cost = card.get("cost")
    if cost is not None:
...
def _fmt_enemy(enemy: dict[str, Any]) -> str:
    """Format an enemy dict as a compact human-readable string."""
    ...
    if intent:
        ...
        if dmg and hits:
            intent_str = (
                f" [{itype} {dmg}x{hits}={dmg * hits}]" if hits > 1 else f" [{itype} {dmg}]"
            )
        else:
            intent_str = f" [{itype}]"
...
def _brief_combat_state(state: GameState) -> str:
    """Return a one-line summary of the current combat state."""
...
def _format_full_state(state: GameState) -> str:
    """Render the full game state as a multi-line human-readable string."""
    lines: list[str] = []
    ...
    lines.append("\n--- Player ---")
    lines.append(
        f"HP: {hp}/{max_hp}  Energy: {energy}" + (f"  Block: {block}" if block else "")
    )
...
def _resolve_potion(potions: list[dict], potion_name: str) -> tuple[int, dict] | None:
    """Find potion by name (case-insensitive, partial match)."""
...
async def get_game_state(client: SpireBridgeClient) -> str:
    """Return the full formatted game state."""
...
async def play_card(
    client: SpireBridgeClient, card_name: str, target: str | None = None
) -> str:
    """Play a card from hand by name, optionally targeting an enemy."""
    state = await client.get_state()
    hand = get_hand(state)
...
async def end_turn(client: SpireBridgeClient) -> str:
...
async def use_potion(
    client: SpireBridgeClient, potion_name: str, target: str | None = None
) -> str:
    """Use a potion by name, optionally targeting an enemy."""
    state = await client.get_state()
    player = get_player(state)
    potions = player.get("potions", [])
...
async def choose_map_node(client: SpireBridgeClient, node_type: str) -> str:
...
async def choose_reward(client: SpireBridgeClient, index: int) -> str:
...
async def choose_card_reward(client: SpireBridgeClient, card_name: str) -> str:
    """Pick a card from the card reward screen by name, or 'skip' to skip."""
    state = await client.get_state()
...
async def rest_site_action(
    client: SpireBridgeClient, action: str, card_name: str | None = None
) -> str:
    ...
    if id_target == "SMITH" and card_name:
        ...
        if match_idx is not None and upgrade_actions:
            ...
            return (
                f"Rested (smith selected) but card '{card_name}' not found in deck for upgrade."
            )

    return f"Performed '{action}' at rest site."


async def choose_event_option(client: SpireBridgeClient, index: int) -> str:
...
async def proceed(client: SpireBridgeClient) -> str:
...
async def start_run(client: SpireBridgeClient, character: str = "Ironclad") -> str:
...
async def abandon_run(client: SpireBridgeClient) -> str:
